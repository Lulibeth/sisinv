...
@session_start();
	...
	if($pagina==0)
		$pagina=1;
	$pagina--;
	$sql="SELECT id, descripcion,(SELECT nombre  FROM public.estatus where id=tip_produ.estatus) as descestatus, usuario, fecha, ip
  FROM factu_inv.tip_produ where
					concat(descripcion,' ')
					ilike '%".$nombre."%' 
	...
	$aux=array(
			'data'=>$data,
			'total'=>$total,
			'path'=>'/vista/tipo_produ_json.php',
			'pagination'=>array(
								'path'=>'/vista/tipo_produ_json.php',
								'current_page'=>$pagina++,
								'per_page'=>$porPagina,
