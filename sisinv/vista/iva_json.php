...
@session_start();
	...
	if($pagina==0)
		$pagina=1;
	$pagina--;
	$sql="SELECT id, descripcion,(SELECT nombre  FROM public.estatus where id=iva.estatus) as descestatus, usuario, fecha, ip, n_calcular
  FROM factu_inv.iva where
					concat(descripcion,' ', n_calcular)
					ilike '%".$nombre."%' 
