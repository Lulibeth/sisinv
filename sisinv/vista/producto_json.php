...
@session_start();
	...
	$pagina--;
	$sql="SELECT id, 
((SELECT descripcion FROM factu_inv.articulo WHERE id=producto.articulo )||' --> '|| 
(SELECT descripcion FROM factu_inv.tip_produ where id=producto.tipo_articulo)||' --> '|| 
(SELECT descripcion FROM factu_inv.talla where id=producto.talla) ) descripcion,
(substring(cast(precio as text),0,(position('€' in cast(precio as text)) -1))||' --> '|| 
(SELECT  descripcion  FROM factu_inv.iva where id=producto.iva) || ' --> '|| 
substring(cast(valort as text),0,(position('€' in cast(valort as text)) -1))) as precio,
...
(SELECT nombre  FROM public.estatus where id=producto.estatus) AS descestatus
  FROM factu_inv.producto
				where
					concat(((SELECT descripcion FROM factu_inv.articulo WHERE id=producto.articulo )||' --> '|| 
(SELECT descripcion FROM factu_inv.tip_produ where id=producto.tipo_articulo)||' --> '|| 
(SELECT descripcion FROM factu_inv.talla where id=producto.talla) ),(substring(cast(precio as text),0,(position('€' in cast(precio as text)) -1))||' --> '|| 
(SELECT  descripcion  FROM factu_inv.iva where id=producto.iva) || ' --> '|| 
substring(cast(valort as text),0,(position('€' in cast(valort as text)) -1))),(SELECT (rif||'-->'||descripcion)  FROM factu_inv.proveedor where id=producto.proveedor))
					ilike '%".$nombre."%' 
