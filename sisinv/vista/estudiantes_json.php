...
@session_start();
	...
	if($pagina==0)
		$pagina=1;
	$pagina--;
	$sql="SELECT p.id,(SELECT id  FROM persona.estudiante where id_persona=p.id) as estudianten, ((SELECT  descripcion  FROM persona.prefijo_cedula WHERE id=p.pre_cedula)||p.cedula) as cedula, 
p.nombres, p.apellidos,(SELECT descripcion  
FROM general.estatus where id=p.id_estatus) as descestatus
FROM persona.persona as p
				where
					concat(((SELECT  descripcion  FROM persona.prefijo_cedula WHERE id=p.pre_cedula)||p.cedula),' ',p.apellidos,' ',p.nombres)
					ilike '%".$nombre."%' 
	...
	$aux=array(
			'data'=>$data,
			'total'=>$total,
			'path'=>'/sisadmac/stssai/vista/preinscritos_json.php',
			'pagination'=>array(
