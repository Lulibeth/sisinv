...
@session_start();
	...
	if($pagina==0)
		$pagina=1;
	$pagina--;
	$sql="SELECT id, descripcion, rif, telefono, telefono_f, (SELECT nombre  FROM public.estatus where id=proveedor.estatus) as descestatus, usuario, 
       ip
  FROM factu_inv.proveedor where
					concat(descripcion,rif,' ',telefono)
					ilike '%".$nombre."%' 
