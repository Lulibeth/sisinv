...
@session_start();
	...
	if($pagina==0)
		$pagina=1;
	$pagina--;
	$sql="SELECT id, descripcion,(SELECT nombre  FROM public.estatus where id=articulo.estatus) as descestatus
  FROM factu_inv.articulo where
					concat(descripcion,' ')
					ilike '%".$nombre."%' 
